...
class VideoProcessHandle:
    ...
    def on_receive_frame(self, frame_id, image, plate, size_fluc, n_box):
        if not self.is_frame_accepted(frame_id, image):
            self.out.write(image)
            return
        #        print "Processing frame {}".format(str(frame_id))
        [objs, is_object_found, img] = self.find_object(image, plate, size_fluc, n_box)
        if is_object_found:
            for obj in objs:
    ...
    def find_object(self, image, plate, size_fluc, n_box):
        # TODO: process all accepted frame here, found the top, left, right, bottom, sign_id and pass to on_object_found
        # Dummy data

        cors = detector.predictor(plate, image, size_fluc, n_box)
        if len(cors) != 0:
            (sign_id, left, top, right, bottom) = cors
    ...
    def on_video_loaded(self, video, output_name):
        ...
        self.out = cv2.VideoWriter(output_name + '.avi', fourcc, fps, (640, 480))
